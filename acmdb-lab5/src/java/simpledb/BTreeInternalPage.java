...
/**
 * Each instance of BTreeInternalPage stores data for one page of a BTreeFile and implements the Page interface that is
 * used by BufferPool.
 *
 * @see BTreeFile
 * @see BufferPool
...
public class BTreeInternalPage extends BTreePage {
    ...
    /**
     * Create a BTreeInternalPage from a set of bytes of data read from disk. The format of a BTreeInternalPage is a set
     * of header bytes indicating the slots of the page that are in use, some number of entry slots, and extra bytes for
     * the parent pointer, one extra child pointer (a node with m entries has m+1 pointers to children), and the
     * category of all child pages (either leaf or internal). Specifically, the number of entries is equal to: <p>
     * floor((BufferPool.getPageSize()*8 - extra bytes*8) / (entry size * 8 + 1))
     * <p> where entry size is the size of entries in this index node
     * (key + child pointer), which can be determined via the key field and {@link Catalog#getTupleDesc}. The number of
     * 8-bit header words is equal to:
     * <p>
     ...
     */
    public BTreeInternalPage(BTreePageId id, byte[] data, int key)
    throws IOException {
        super(id, key);
    ...
    void checkRep(Field lowerBound, Field upperBound, boolean checkOccupancy, int depth) {
    ...
    int getMaxEntries() {
        ...
        return (BufferPool.getPageSize() * 8 - extraBits) / bitsPerEntryIncludingHeader; //round down
    }
     ...
     */
    private Field readNextKey(DataInputStream dis, int slotId)
     ...
     */
    private int readNextChild(DataInputStream dis, int slotId)
    throws NoSuchElementException {
     ...
     * BTreeInternalPage constructor and have it produce an identical BTreeInternalPage object.
     *
    ...
    public byte[] getPageData() {
        ...
        int zerolen = BufferPool.getPageSize() -
                      (INDEX_SIZE + 1 + header.length + td.getFieldType(keyField).getLen() * (keys.length - 1) +
    ...
    /**
     * Delete the specified entry (key + 1 child pointer) from the page. The recordId is used to find the specified
     * entry, so it must not be null. After deletion, the entry's recordId should be set to null to reflect that it is
    ...
    /**
     * Delete the specified entry (key + right child pointer) from the page. The recordId is used to find the specified
     * entry, so it must not be null. After deletion, the entry's recordId should be set to null to reflect that it is
     * no longer stored on any page.
     *
     * @param e The entry to delete
    ...
    /**
     * Delete the specified entry (key + left child pointer) from the page. The recordId is used to find the specified
     * entry, so it must not be null. After deletion, the entry's recordId should be set to null to reflect that it is
     * no longer stored on any page.
     *
     * @param e The entry to delete
     ...
     * @throws DbException if this entry is not on this page, entry slot is already empty, or updating this key
     *         would put the entry out of order on the page
     */
    ...
    /**
     * Adds the specified entry to the page; the entry's recordId should be updated to reflect that it is now stored on
     * this page.
     *
     * @param e The entry to add.
     * @throws DbException if the page is full (no empty slots) or key field type, table id, or child page
     *         category is a mismatch, or the entry is invalid
     */
     ...
     *         UnsupportedOperationException) (note that this iterator shouldn't return entries in empty slots!)
     */
    ...
    /**
     * @return a reverse iterator over all entries on this page (calling remove on this iterator throws an
     *         UnsupportedOperationException) (note that this iterator shouldn't return entries in empty slots!)
     */
    Iterator<BTreeEntry> reverseIterator() {
...
class BTreeInternalPageIterator implements Iterator<BTreeEntry> {
    private final BTreeInternalPage p;
...
class BTreeInternalPageReverseIterator implements Iterator<BTreeEntry> {
    ...
    private BTreePageId childId = null;
