...
public class BTreeChecker {
    ...
    /**
     * checks the integrity of the tree: 1) parent pointers. 2) sibling pointers. 3) range invariants. 4) record to page
     * pointers. 5) occupancy invariants. (if enabled)
     */
    ...
    private static SubtreeSummary checkSubTree(BTreeFile bt, TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        if (page.getId().pgcateg() == BTreePageId.LEAF) {
            ...
            while (it.hasNext()) {
                ...
                SubtreeSummary currentSubTreeResult = checkSubTree(bt, tid, dirtypages, curr.getLeftChild(), lowerBound,
                                                                   curr.getKey(), ipage.getId(), checkOccupancy,
                                                                   depth + 1);
                assert currentSubTreeResult != null;
    ...
    static class SubtreeSummary {
        int depth;
        BTreePageId ptrLeft;
        BTreePageId leftmostId;
        BTreePageId ptrRight;
        BTreePageId rightmostId;
