...
/**
 * LogicalPlan represents a logical query plan that has been through the parser and is ready to be processed by the
 * optimizer.
 * <p>
 * A LogicalPlan consists of a collection of table scan nodes, join nodes, filter nodes, a select list, and a group by
 * field. LogicalPlans can only represent queries with one aggregation field and one group by field.
 * <p>
 * LogicalPlans can be converted to physical (optimized) plans using the {@link #physicalPlan} method, which uses the
 * {@link JoinOptimizer} to order joins optimally and to select the best implementations for joins.
 */
public class LogicalPlan {
    private final Vector<LogicalScanNode> tables;
    private final Vector<LogicalFilterNode> filters;
    private final HashMap<String, DbIterator> subplanMap;
    private final HashMap<String, Integer> tableMap;
    private final Vector<LogicalSelectListNode> selectList;
     ...
     */
    private static Aggregator.Op getAggOp(String s)
    ...
    /**
     * Set the text of the query representing this logical plan.  Does NOT parse the specified query -- this method is
     * just used so that the object can print the SQL it represents.
     *
     ...
     *         (tablename.field or alias.field), or can be a unique field name without a tablename qualifier.  If it is
     *         an ambiguous name, it will throw a ParsingException
     * @param p The predicate for the filter
     * @param constantValue the constant to compare the predicate against; if field is an integer field, this
     *         should be a String representing an integer
     * @throws ParsingException if field is not in one of the tables added via {@link #addScan} or if field is
     *         ambiguous (e.g., two tables contain a field named field.)
     */
    void addFilter(String field, Predicate.Op p, String constantValue)
    throws ParsingException {
     ...
     *         tableName.field or alias.field) or may be an unqualified unique field name.  If the name is ambiguous or
     *         unknown, a ParsingException will be thrown.
     * @param joinField2 The name of the second join field
     * @param pred The join predicate
     * @throws ParsingException if either of the fields is ambiguous, or is not in one of the tables added via
     *         {@link #addScan}
     */
     ...
     *         tableName.field or alias.field) or may be an unqualified unique field name.  If the name is ambiguous or
     *         unknown, a ParsingException will be thrown.
     * @param joinField2 the subquery to join with -- the join field of the subquery is the first field in the
     *         result set of the query
     * @param pred The join predicate.
     * @throws ParsingException if either of the fields is ambiguous, or is not in one of the tables added via
     *         {@link #addScan}
     */
    void addJoin(String joinField1, DbIterator joinField2, Predicate.Op pred)
    throws ParsingException {
        joinField1 = disambiguateName(joinField1);
    ...
    /**
     * Add a scan to the plan. One scan node needs to be added for each alias of a table accessed by the plan.
     *
    ...
    /**
     * Add a specified field/aggregate combination to the select list of the query. Fields are output by the query such
     * that the rightmost field is the first added via addProjectField.
     *
    ...
    /**
     * Add an aggregate over the field with the specified grouping to the query.  SimpleDb only supports a single
     * aggregate expression and GROUP BY field.
     *
    ...
    /**
     * Add an ORDER BY expression in the specified order on the specified field.  SimpleDb only supports a single ORDER
     * BY field.
     *
    ...
    /**
     * Given a name of a field, try to figure out what table it belongs to by looking through all of the tables added
     * via {@link #addScan}.
     *
     ...
     *
     * @throws ParsingException if the field cannot be found in any of the tables, or if the field is ambiguous
            ...
            try {
                ...
                if (tableName == null) {
                    ...
                    throw new ParsingException(
                            "Field " + name + " appears in multiple tables; disambiguate by referring to it as " +
                            "tablename." + name);
                }
            }
    ...
    /**
     * Convert this LogicalPlan into a physicalPlan represented by a {@link DbIterator}.  Attempts to find the optimal
     * plan by using {@link JoinOptimizer#orderJoins} to order the joins in the plan.
     *
     * @param t The transaction that the returned DbIterator will run as a part of
     * @param baseTableStats a HashMap providing a {@link TableStats} object for each table used in the
     *         LogicalPlan.  This should have one entry for each table referenced by the plan, not one entry for each
     *         table alias (so a table t aliases as t1 and t2 would have just one entry with key 't' in this HashMap).
        ...
        if (hasAgg) {
            ...
            try {
                aggNode = new Aggregate(node, td.fieldNameToIndex(aggField),
                                        ...
                                        td.fieldNameToIndex(groupByField), getAggOp(aggOp));
            }
