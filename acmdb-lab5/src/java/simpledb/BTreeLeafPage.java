...
/**
 * Each instance of BTreeLeafPage stores data for one page of a BTreeFile and implements the Page interface that is used
 * by BufferPool.
 *
 * @see BTreeFile
 * @see BufferPool
...
public class BTreeLeafPage extends BTreePage {
    ...
    /**
     * Create a BTreeLeafPage from a set of bytes of data read from disk. The format of a BTreeLeafPage is a set of
     * header bytes indicating the slots of the page that are in use, and some number of tuple slots, as well as some
     * extra bytes for the parent and sibling pointers. Specifically, the number of tuples is equal to: <p>
     ...
     */
    public BTreeLeafPage(BTreePageId id, byte[] data, int key)
    throws IOException {
        super(id, key);
    ...
    void checkRep(int fieldId, Field lowerBound, Field upperBound, boolean checkOccupancy, int depth) {
    ...
    throws NoSuchElementException {
        ...
        // return null.
        return Utility.readNextTuple(dis, header, slotId, td, pid);
    }
     ...
     * BTreeLeafPage constructor and have it produce an identical BTreeLeafPage object.
     *
    ...
    /**
     * Adds the specified tuple to the page such that all records remain in sorted order; the tuple should be updated to
    ...
    /**
     * @return a reverse iterator over all tuples on this page (calling remove on this iterator throws an
     *         UnsupportedOperationException) (note that this iterator shouldn't return tuples in empty slots!)
     */
    Iterator<Tuple> reverseIterator() {
