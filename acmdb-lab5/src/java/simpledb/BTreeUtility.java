...
public class BTreeUtility {
    ...
    private static ArrayList<Integer> tupleToList(Tuple tuple) {
     ...
     *         3)
     */
    private static Tuple getBTreeTuple(int[] tupdata) {
     ...
     *         3)
     */
    private static Tuple getBTreeTuple(ArrayList<Integer> tupdata) {
    ...
    /**
     * @return a Tuple with a 'width' IntFields each with value n and with RecordId(BTreePageId(1, 2, BTreePageId
     *         .LEAF), 3)
     */
    ...
    static BTreeEntry getBTreeEntry(int n) {
     ...
     *         BTreePageId.INTERNAL), 3)
     */
    static BTreeEntry getBTreeEntry(int n, int key, int tableid) {
     ...
     */
    @SuppressWarnings("SameParameterValue")
    private static BTreeFile createRandomBTreeFile(int columns, int rows, int maxValue,
    ...
    throws IOException, DbException, TransactionAbortedException {
        ...
        return convertTupleListToBTreeFile(columns, tuples, keyField);
    }

    /**
     * Convert the tuples list to a B+ tree file.
     */
    private static BTreeFile convertTupleListToBTreeFile(int columns, ArrayList<ArrayList<Integer>> tuples,
                                                         int keyField)
    throws IOException, DbException, TransactionAbortedException {
     ...
     */
    private static void generateRandomTuples(int columns, int rows, int maxValue,
                                             Map<Integer, Integer> columnSpecification,
     ...
     * @param tuples - list of tuples to return
     */
    private static void generateRandomTuples(int columns, int rows, int minValue, int maxValue,
                                             Map<Integer, Integer> columnSpecification,
                                             ArrayList<ArrayList<Integer>> tuples) {
     ...
     */
    private static void generateRandomEntries(int numKeys, int minKey, int maxKey, int minChildPtr,
     ...
     */
    private static ArrayList<Tuple> generateRandomTuples(int columns, int rows, int min, int max) {
        ArrayList<ArrayList<Integer>> tuples = new ArrayList<>(rows);
     ...
     */
    private static ArrayList<BTreeEntry> generateRandomEntries(int numKeys, int tableid, int childPageCategory,
     ...
     */
    @SuppressWarnings("SameParameterValue")
    static BTreeLeafPage createRandomLeafPage(BTreePageId pid, int columns, int keyField, int min, int max)
    throws IOException {
     ...
     * @throws IOException
     */
    static BTreeLeafPage createRandomLeafPage(BTreePageId pid, int columns, int keyField, int numTuples, int min,
                                              int max)
    throws IOException {
        Type[] typeAr = new Type[columns];
        Arrays.fill(typeAr, Type.INT_TYPE);
    ...
    static int getNumEntriesPerPage() {
        ...
        return (BufferPool.getPageSize() * 8 - internalpointerbytes * 8 - 1) /
               (nentrybytes * 8 + 1);  //floor comes for free
    }
     ...
     */
    @SuppressWarnings("SameParameterValue")
    static BTreeInternalPage createRandomInternalPage(BTreePageId pid, int keyField, int childPageCategory, int minKey,
     ...
     * @throws IOException
     */
    static BTreeInternalPage createRandomInternalPage(BTreePageId pid, int keyField, int childPageCategory, int numKeys,
                                                      int minKey, int maxKey, int minChildPtr)
    throws IOException {
        byte[] data = BTreeFileEncoder.convertToInternalPage(
                BTreeUtility.generateRandomEntries(numKeys, pid.getTableId(), childPageCategory, minKey, maxKey,
                                                   minChildPtr), BufferPool.getPageSize(), Type.INT_TYPE,
                childPageCategory);
     ...
     */
    @SuppressWarnings("SameParameterValue")
    static BTreeFile createEmptyBTreeFile(String path, int cols, int keyField, int pages)
    throws IOException {
        File f = new File(path);
    ...
    static class BTreeWriter extends Thread {
         ...
         */
        BTreeWriter(TransactionId tid, BTreeFile bf, int item, int count) {
            this.tid = tid;
            this.bf = bf;
            this.item = item;
            this.count = count;
