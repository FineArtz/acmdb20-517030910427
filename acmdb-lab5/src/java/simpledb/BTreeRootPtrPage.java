...
/**
 * BTreeRootPtrPage stores the pointer to the root node used in the B+ tree and implements Page Interface that is used
 * by BufferPool
 *
 * @see BufferPool
 */
public class BTreeRootPtrPage implements Page {
    ...
    /**
     * Constructor. Construct the BTreeRootPtrPage from a set of bytes of data read from disk. The format of an
     * BTreeRootPtrPage is an integer for the page number of the root node, followed by a byte to encode the category of
     * the root page (either leaf or internal), followed by an integer for the page number of the first header page
     */
    public BTreeRootPtrPage(BTreePageId id, byte[] data)
    throws IOException {
        this.pid = id;
    ...
    /**
     * There is only one instance of a BTreeRootPtrPage per table. This static method is separate from getId() in order
     * to maintain the Page interface
     *
    ...
    /**
     * Static method to generate a byte array corresponding to an empty BTreeRootPtrPage. Used to add new, empty pages
     * to the file. Passing the results of this method to the BTreeRootPtrPage constructor will create a
    ...
    /**
     * Generates a byte array representing the contents of this root pointer page. Used to serialize this root pointer
     * page to disk. The invariant here is that it should be possible to pass the byte array generated by getPageData to
     * the BTreeRootPtrPage constructor and have it produce an identical BTreeRootPtrPage object.
     *
     ...
     * @throws DbException if the id is invalid
     */
    void setHeaderId(BTreePageId id)
    throws DbException {
        if (id == null) {
            header = 0;
        }
