...
public class BTreeFileEncoder {
    ...
    public static BTreeFile convert(ArrayList<ArrayList<Integer>> tuples, File hFile, File bFile, int keyField,
                                    int numFields)
    throws IOException {
        return convert(innerConvert(tuples, numFields), hFile, bFile, keyField, numFields);
    }

    private static File innerConvert(ArrayList<ArrayList<Integer>> tuples, int numFields)
    throws IOException {
     ...
     * @return the BTreeFile
     */
    private static BTreeFile convert(File inFile, File hFile, File bFile, int keyField, int numFields)
    throws IOException {
    ...
    throws IOException, DbException, TransactionAbortedException {
        return convert(innerConvert(tuples, numFields), hFile, bFile, npagebytes, numFields, typeAr, fieldSeparator,
                       keyField);
    }
    ...
    private static BTreeFile convert(File inFile, File hFile, File bFile, int npagebytes, int numFields, Type[] typeAr,
                                     char fieldSeparator, int keyField)
    throws IOException, DbException, TransactionAbortedException {
        // convert the inFile to HeapFile first.
        HeapFileEncoder.convert(inFile, hFile, BufferPool.getPageSize(), numFields);
        HeapFile heapf = Utility.openHeapFile(numFields, hFile);
        ...
        if (page2.size() == 0) {
            ...
            ArrayList<Tuple> secondToLastPg = new ArrayList<>(page1.subList(0, remainingTuples / 2));
    ...
    private static void cleanUpEntries(ArrayList<ArrayList<BTreeEntry>> entries, BTreeFile bf, int nentries,
                                       int npagebytes, Type keyType, int tableid, int keyField)
    throws IOException {
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            if (size <= nentries) {
                ...
                ArrayList<BTreeEntry> secondToLastPg = new ArrayList<>(entries.get(i).subList(0, size / 2));
                ArrayList<BTreeEntry> lastPg = new ArrayList<>(entries.get(i).subList(size / 2 + 1, size));
    ...
    throws IOException {
        ...
        if (size > 0) {
            ...
            if (size == nentries * 2 + 1) {
                ...
                ArrayList<BTreeEntry> remainingEntries = new ArrayList<>(
     ...
     */
    @SuppressWarnings("JavaDoc")
    static byte[] convertToLeafPage(ArrayList<Tuple> tuples, int npagebytes, int numFields, Type[] typeAr, int keyField)
    throws IOException {
        int nrecbytes = 0;
        for (int i = 0; i < numFields; i++) {
            nrecbytes += typeAr[i].getLen();
     ...
     */
    @SuppressWarnings("JavaDoc")
    static byte[] convertToInternalPage(ArrayList<BTreeEntry> entries, int npagebytes, Type keyType,
     ...
     */
    @SuppressWarnings("JavaDoc")
    static byte[] convertToRootPtrPage(int root, int rootCategory, int header)
    throws IOException {
